...
class ${name} extends AvenxComponent {
  ...
  processForLoops(template) {
    ...
    while (true) {
      ...
      if (!innerPair) {
        logger.warn('[ComponentParser] Unmatched <@for> tags in template.');
        break;
      }
  ...
  processComponentTags(template) {
    ...
    while (true) {
      ...
      if (isSelfClosing) {
        const replacement = `<div data-avenx-comp="${compName}"${propsAttr}${othersAttr}></div>`;
        currentTemplate =
        ...
        if (closingTagIndex === -1) {
          ...
          const replacement = `<div data-avenx-comp="${compName}"${propsAttr}${othersAttr}>${processedInner}</div>`;
          currentTemplate =
  ...
  processTransitionTags(template) {
    try {
      ...
      logger.warn('[ComponentParser Warning] Failed to parse transition tags:', err);
      return template;
    }
  }
  ...
  optimizeStaticSubtrees(template) {
    try {
      ...
      logger.warn('[ComponentParser Warning] Failed to optimize static subtrees:', err);
      return template;
    }
  }
